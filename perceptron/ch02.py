...
class Perceptron:
    ...
    def fit(self, X, y):
        ...
        self.w_ = rgen.normal(loc=0.0, scale=0.01, size=X.shape[1]) ## shape[0] --> n_examples e shape[1] --> n_features
        # self.b_ = np.float_(0.)
...
# print('arccos(...)', np.arccos(v1.dot(v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))))
...
try:
    ...
    # s = "C:\Users\elpen\Documents\LAVORI\Scuola\Didattica\PYTHON_WS\ch02"
    s = "/workspaces/ITS_AI_Specialist/perceptron/iris.data"
    print('From URL:', s)
    df = pd.read_csv(s,
                     header=None,
                     encoding='utf-8')
...
plt.savefig('/workspaces/ITS_AI_Specialist/perceptron/images/02_06_irisdata.png', dpi=300)
plt.show()
...
plt.savefig('/workspaces/ITS_AI_Specialist/perceptron/images/02_07_training errors.png', dpi=300)
plt.show()
...
plt.savefig('/workspaces/ITS_AI_Specialist/perceptron/images/02_08_ decision regions.png', dpi=300)
plt.show()
